...
| Hero | Inside Tame Impala’s mind; *Currents* (2015) | Original large face and brain cutaway, poking out from the bottom; a bundle of strands bursting from the cut and fanning into the upper-left background | `scripts/art.py` → `brain_portrait()` clips the original portrait into the shared scene as a static image; `brain_lines()` draws the inline strand layer behind it, launched parallel to the lid's cut edge and swayed by page CSS; the title uses difference blending |
...
- Asset links carry a content hash (`style.css?v=…`, `app.js?v=…`, `art/currents.svg?v=…`, `art/brain-portrait.svg?v=…`, portrait rasters and `favicon.png`) so readers see new styles after a deploy. `scripts/check.py` verifies the hashes match the files and that only the two portrait rasters and face/brain favicon ship.
...
- Motion includes disclosure expansion and collapse, button hover, the agent caret turning and the slow sway of the hero strands; these stop under `prefers-reduced-motion`. Never autoplay audio or video.
...
Each strand sways about its own rim origin (an inline `transform-origin` in scene units with `transform-box: view-box`), ±0.7° over a 9 s alternating cycle with an 80 ms stagger per strand, so a slow ripple travels across the fan. The pivots stay fixed inside the head, keeping the strands attached while the face remains still. `prefers-reduced-motion` stops the sway. Motion needs no JavaScript; when CSS animation is unavailable, the static geometry remains.
