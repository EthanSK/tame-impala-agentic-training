...
- Hero height must never be blank. Ethan rejected a taller hero whose extra space sat empty above a bottom-anchored head. Keep the head beside the copy and spend extra height below the neck: the elongated figure scales uniformly from the hero height (never a vertical stretch of the face) and the generated ribbon band plus dark lip absorb the rest, running into the Currents strip. Add no spacer elements or bottom anchoring that leaves a gap.
- When blending a raster into the hero, apply the blend mode to the figure container: a positioned stacking context can isolate a child image and leave its cream rectangle visible. Mask the sides as well as the bottom, and keep continuation ribbon curves parallel so the join does not become crossing lines. Verify the actual join at narrow and tall widths.
