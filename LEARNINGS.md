...
- Keep the animated strands as inline vector markup in the page and the portrait as a separate static image in the same scene box. Page CSS can then sway each strand about its own inline `transform-origin` and honour `prefers-reduced-motion`; animation inside an `<img>` SVG cannot be controlled or reliably reduced from the page.
