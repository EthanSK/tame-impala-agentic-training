...
- Keep the original large portrait at the bottom edge. The current visual direction replaces the elongated torso with brain-originating Currents lines. Draw portrait and waves in shared SVG coordinates so the lines stay attached to the brain at every size.
- Anchor the strands to the picture's actual cut geometry, not a hovering cone: origins along the skull's front rim, launch parallel to the lifted lid's edge, so every strand surfaces through the exposed brain arc. Ethan rejected a separate upward mobile fan; the emission angle must match desktop on every screen, and a narrow phone lets the shallow strands leave the left edge instead.
- Keep the portrait as a separate static image and the SVG paths as the shared geometry source and fallback. A ±0.7° slow rotation produced pixel differences but did not read as animation to Ethan. The canvas renderer now deforms the strands with traveling waves while locking the opening; verify perceptible shape changes over a short interval, not just unequal screenshot hashes. Respect reduced motion and stop drawing when the hero is offscreen or the document is hidden.
- Extra scene height does not fix finite path endpoints: extend both the strand paths and their filled envelope, allow SVG overflow, and clip only at the hero edge. Verify narrow, tall layouts (including 280×1200) as well as standard phones and desktop; keep the portrait size and diagonal emission independent of the extra coverage.
